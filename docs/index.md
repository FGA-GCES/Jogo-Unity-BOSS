# 🎮 Diários de Sofia - Unity for Women



**Projeto Open Source de Desenvolvimento de Jogos com Unity**  

*Universidade de Brasília (UnB) - GCES 2025.1*

![Unity](https://img.shields.io/badge/Unity-100000?style=for-the-badge&logo=unity&logoColor=white)
![C#](https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=c-sharp&logoColor=white)
![Open Source](https://img.shields.io/badge/Open%20Source-❤️-red?style=for-the-badge)
![MIT License](https://img.shields.io/badge/License-MIT-yellow?style=for-the-badge)




---

## 🌟 Sobre o Projeto

O **Diários de Sofia** é um projeto educacional open source desenvolvido para promover **diversidade e inclusão na tecnologia** através do desenvolvimento de jogos. Criado como parte da disciplina de **Gerenciamento de Configuração e Evolução de Software (GCES)** da UnB, este projeto combina:

- 🎯 **Aprendizado prático** de desenvolvimento de jogos com Unity
- 👥 **Colaboração em equipe** usando metodologias ágeis
- 🔧 **Boas práticas** de engenharia de software
- 🌍 **Código aberto** para beneficiar a comunidade

### 🎮 O Jogo

"Diários de Sofia" é um **RPG narrativo 2D** em pixel art que combina puzzles e combate para conscientizar sobre igualdade e justiça social. Com perspectiva top-down inspirada em clássicos, o jogo oferece uma experiência envolvente que vai além do entretenimento.

---

## 📚 Navegação da Documentação

- [📖 Sobre o Jogo](./jogo/game.md) - História, mecânicas e conceitos
- [🎮 Guia de Gameplay](./jogo/scripts.md) - Como jogar
- [📋 Backlog](./jogo/backlog.md) - Funcionalidades planejadas
- [🚀 Onboarding](./ONBOARDING.md) - Primeiros passos no projeto
- [🤝 Como Contribuir](./CONTRIBUTING.md) - Guia de contribuição
- [📐 Padrões de Código](./jogo/clean_code.md) - Boas práticas
- [🔧 Configuração de Ambiente](./tutoriais/workshop_ambiente.md)
- [📊 Processo de Desenvolvimento](./projeto/devprocess.md)
- [📈 Sprints](./projeto/sprints.md)
- [🛣️ Roadmaps](./projeto/desenvolvimento_roadmap.md)

### 🎓 Tutoriais e Workshops
- [🏠 Workshop Home](./tutoriais/wokshop_home.md)
- [🎮 Desenvolvendo Níveis](./tutoriais/workshop_desenvolvendo_nivel.md)
- [🤖 Inimigos e UI](./tutoriais/workshop_inimigos_ui.md)
- [🚶 Movimento do Player](./tutoriais/workshop_movimento_player.md)

---

## 🎯 Objetivos do Projeto

### 📚 **Aprendizado**
- Aplicar conceitos de GCES na prática
- Dominar ferramentas de controle de versão
- Implementar CI/CD e DevOps
- Praticar metodologias ágeis

### 🌍 **Impacto Social**
- Promover diversidade na tecnologia
- Criar conteúdo educativo sobre igualdade
- Formar uma comunidade inclusiva
- Compartilhar conhecimento através do código aberto

### 🛠️ **Técnico**
- Desenvolver um jogo completo em Unity
- Implementar arquitetura limpa e escalável
- Documentar processos e decisões
- Criar pipelines de qualidade de código

---

## 🚀 Como Começar

### 👀 **Quero apenas jogar**
1. Acesse as [releases](./projeto/releases/) do projeto
2. Baixe a versão mais recente
3. Execute o jogo e divirta-se!

### 💻 **Quero contribuir**
1. Leia o [Código de Conduta](./CODE_OF_CONDUCT.md)
2. Siga o [Guia de Contribuição](./CONTRIBUTING.md)
3. Configure seu [ambiente de desenvolvimento](./projeto/dificuldades_subir_ambiente.md)
4. Escolha uma [issue](./jogo/good_issues.md) e comece a contribuir!

### 📖 **Quero aprender**
1. Explore nossos [tutoriais](./tutoriais/README.md)
2. Participe dos [workshops](./tutoriais/wokshop_home.md)
3. Veja nosso [processo de desenvolvimento](./projeto/devprocess.md)
4. Acompanhe nossos [roadmaps](./projeto/desenvolvimento_roadmap.md)

---

## 📈 Status do Projeto

- ✅ **Fase de Desenvolvimento Ativo**
- 🎮 **Jogo Jogável** com mecânicas básicas
- 📚 **Documentação Completa** em construção
- 🤝 **Aceitando Contribuições** da comunidade
- 🎓 **Material Educativo** em desenvolvimento

---

## 🤝 Comunidade

### 💬 **Conecte-se conosco**
- 📧 Entre em contato através das [issues](https://github.com/UnB-Games/Jogo-Unity-BOSS/issues)
- 🐛 Reporte bugs ou sugira melhorias
- 💡 Compartilhe suas ideias
- 📖 Contribua com a documentação

### 🏆 **Reconhecimentos**
- **Disciplina:** Gerenciamento de Configuração e Evolução de Software
- **Instituição:** Universidade de Brasília (UnB)
- **Semestre:** 2025.1
- **Tema:** Unity for Women - Diversidade na Tecnologia

---

## 📄 Licença

Este projeto está licenciado sob a [Licença MIT](./LICENSE) - veja o arquivo para detalhes.

**Código aberto, educação livre, impacto social positivo!** 💜

---

<div align="center">

*Desenvolvido com ❤️ pela comunidade Unity for Women*  
*Universidade de Brasília - 2025*

</div>